...
import { AMSTERDAM, CATEGORIES } from "@/app/data/amsterdam-places";
...
const HERO = "/images/destinations/flights-amsterdam.avif";
const usdMonths = AMSTERDAM.monthlyPrices.map((m) => ({ month: m.month, price: usd5(m.price) }));
...
export function generateMetadata(): Metadata {
  ...
  const title = clampTitle(`Cheap Flights to Amsterdam ${year} — Guide, Prices & Attractions | Flyamba`);
  const description = clampDescription(`Find cheap flights to Amsterdam, Netherlands from $${MIN_USD}, plus complete English guides to attractions, restaurants, hotels, transport, weather, shopping, nightlife, family travel and day trips. Canals, world museums and easy-going charm.`);
  const canonical = `${SITE}/amsterdam`;
  return {
    title,
    description,
...
const FAQS: FaqItem[] = [
  {
    q: "How much are flights to Amsterdam?",
    a: `Round-trip fares to Amsterdam Schiphol (AMS) start from around $${MIN_USD} and average roughly $${MAX_USD} in peak summer. February and November are the cheapest months to fly, while July and August are the most expensive.`,
  },
  {
    q: "When is the cheapest time to fly to Amsterdam?",
    a: "February and November are the cheapest months, with fares from about $255 round trip. Booking 6–8 weeks ahead and flying midweek (Tuesday or Wednesday) usually gets the best price.",
  },
  {
  ...
  },
  {
    ...
    a: "April and May bring tulip season and Keukenhof, plus lively King's Day; summer (around 20°C) is warmest but busiest. Spring and early autumn offer the best balance of decent weather, blooms and lower prices.",
  },
];

function jsonLd() {
  const url = `${SITE}/amsterdam`;
  const touristDestination = {
    "@context": "https://schema.org",
    "@type": "TouristDestination",
    name: "Amsterdam",
    description: AMSTERDAM.tagline,
...
const NON_STOP = [
  ...
  { city: "Boston", price: 359, iata: "BOS" },
  { city: "Atlanta", price: 445, iata: "ATL" },
  { city: "Dubai", price: 399, iata: "DXB" },
];
...
const ATTRACTION_PREVIEW = [
  { name: "Rijksmuseum", blurb: "Rembrandt's Night Watch and 800 years of Dutch art.", image: "/images/amsterdam/sevardheter/rijksmuseum.webp" },
  { name: "Anne Frank House", blurb: "The moving canal-side hiding place and diary museum.", image: "/images/amsterdam/sevardheter/anne-franks-hus.webp" },
  { name: "The Canal Ring", blurb: "UNESCO waterways, gabled houses and 1,500 bridges.", image: "/images/amsterdam/sevardheter/canal-ring-grachtengordel.webp" },
];
const EAT_PREVIEW = [
  { name: "Moeders", blurb: "Hearty traditional Dutch cooking under a wall of photos.", image: "/images/amsterdam/restauranger/moeders.webp" },
  { name: "Foodhallen", blurb: "20+ global street-food stalls in an old tram depot.", image: "/images/amsterdam/restauranger/foodhallen.webp" },
...
const WHY = [
  { icon: "🎨", text: "World-class museums — the Rijksmuseum, Van Gogh and Anne Frank House rank among Europe's very best." },
  { icon: "🚲", text: "A compact, flat, bike-friendly city where canals, cafés and sights are all within easy walking or cycling distance." },
  { icon: "🌷", text: "Spring tulips and Keukenhof, summer canal-side terraces and a famously open, easy-going atmosphere year-round." },
  { icon: "🌙", text: "Legendary nightlife — from Paradiso and the Melkweg to cosy brown cafés and a serious cocktail scene." },
];
...
export default function AmsterdamHub() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative isolate h-[80vh] min-h-[560px] w-full overflow-hidden">
        ...
        <SmartImage src={HERO} alt="Cheap flights to Amsterdam, Netherlands" fill fetchPriority="high" loading="eager" sizes="100vw" className="object-cover" />
        ...
        <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col justify-end px-4 pb-14 pt-24 sm:px-6 lg:px-8">
          ...
          <div className="mb-4">
            <Breadcrumbs onDark items={crumbsForSlug("amsterdam")} />
          </div>
          <div className="flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-[0.25em] text-white/85">
            <span className="text-base">{AMSTERDAM.countryFlag}</span>
            <span>{AMSTERDAM.country}</span>
            <span className="opacity-40">·</span>
            <span className="rounded-full border border-white/25 bg-white/10 px-2.5 py-0.5 tracking-[0.2em] backdrop-blur">{AMSTERDAM.iata}</span>
            <span className="opacity-40">·</span>
            <span className="rounded-full border border-white/25 bg-white/10 px-2.5 py-0.5 backdrop-blur">{AMSTERDAM.summerTemp}°C summer</span>
          </div>
          <h1 className="mt-4 max-w-4xl font-serif text-5xl font-semibold text-white sm:text-7xl">Cheap Flights to Amsterdam</h1>
          <p className="mt-4 max-w-xl text-lg text-white/85">{AMSTERDAM.tagline}</p>
        </div>
      </section>
      ...
      <section className="relative z-10 mx-auto mt-8 max-w-5xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 rounded-full border border-border bg-card px-6 py-4 text-sm font-medium text-foreground shadow-elegant">
          ...
          <span>KLM hub · nonstop worldwide</span>
          <span className="text-muted-foreground/40">•</span>
          <span>~1h from London · 8h from New York</span>
          <span className="text-muted-foreground/40">•</span>
          <span className="inline-flex items-center gap-1"><Plane className="h-4 w-4 text-accent" /> {AMSTERDAM.iata}</span>
        </div>
      </section>

      {/* Sticky sub-nav */}
      <div className="mt-8">
        <CitySubNav citySlug="amsterdam" categories={CATEGORIES} active="" />
      </div>

      {/* 3. Flight search widget */}
      <section id="flights" className="mx-auto mt-10 max-w-7xl scroll-mt-32 px-4 sm:px-6 lg:px-8">
        <h2 className="mb-3 font-serif text-2xl font-semibold text-foreground sm:text-3xl">Find the Best Flights to Amsterdam</h2>
        <p className="mb-6 max-w-3xl text-muted-foreground">Flying to Amsterdam is easier than ever, with direct routes from New York, London and other major hubs — making Amsterdam one of the most popular flight destinations from the US, UK and Europe. Find cheap flights to Amsterdam, compare airlines and book direct. Our AI flight search compares hundreds of routes to find you the cheapest flights to Amsterdam — just describe your trip and Flyamba does the rest.</p>
        <AviasalesWidget toName={AMSTERDAM.tpName} />

        {/* Observed fares by month. Renders nothing below three months —
            see app/components/FareCalendarSection.tsx. */}
        <FareCalendarSection slug="amsterdam" name="Amsterdam" />
        <LowFareCta slug="amsterdam" city="Amsterdam" />
      </section>

      {/* 4. Booking insights */}
      <section className="mx-auto mt-12 max-w-7xl px-4 sm:px-6 lg:px-8">
        <p className="text-xs font-semibold uppercase tracking-[0.25em] text-accent">Booking insights</p>
        <h2 className="mt-2 font-serif text-3xl font-semibold text-foreground sm:text-4xl">Smart tips for booking Amsterdam</h2>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { icon: Route, label: "Direct flights", value: "Yes — from New York, Atlanta, London" },
          ].map((s) => (
      ...
      <section id="explore" className="mx-auto mt-14 max-w-7xl scroll-mt-32 px-4 sm:px-6 lg:px-8">
        ...
        <h2 className="mt-2 font-serif text-3xl font-semibold text-foreground sm:text-4xl">Explore Amsterdam</h2>
        <div className="mt-8 grid grid-cols-2 gap-4 lg:grid-cols-3">
          {categories.map((c) => (
            ...
            >
              <SmartImage src={CATEGORY_IMAGES[c.slug] ?? HERO} alt={`Amsterdam ${c.label}`} fill sizes="(max-width:1024px) 50vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
      ...
      <section id="cheapest-months" className="mx-auto mt-16 max-w-7xl scroll-mt-32 px-4 sm:px-6 lg:px-8">
        ...
        <h2 className="mt-2 font-serif text-3xl font-semibold text-foreground sm:text-4xl">When is it cheapest to fly to Amsterdam?</h2>
      ...
      <section id="nonstop" className="mx-auto mt-16 max-w-7xl scroll-mt-32 px-4 sm:px-6 lg:px-8">
        ...
        <h2 className="mt-2 font-serif text-3xl font-semibold text-foreground sm:text-4xl">Non-stop to Amsterdam from major cities</h2>
          ...
          {NON_STOP.map((r) => (
            <div key={r.city} className="flex items-center justify-between rounded-3xl border border-border bg-card p-6">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{r.iata} → AMS · nonstop</p>
              </div>
      ...
      <section className="mx-auto mt-16 max-w-7xl px-4 sm:px-6 lg:px-8">
        <p className="text-xs font-semibold uppercase tracking-[0.25em] text-accent">Why Amsterdam?</p>
        <h2 className="mt-2 font-serif text-3xl font-semibold text-foreground sm:text-4xl">Why Fly to Amsterdam with Flyamba?</h2>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {WHY.map((w) => (
      ...
      <section className="mx-auto mt-16 max-w-4xl px-4 sm:px-6 lg:px-8">
        <AskAiWidget destination="Amsterdam" />
      </section>
      ...
      <section className="mx-auto mt-16 max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href="/amsterdam/attractions" className="inline-flex items-center gap-1.5 text-sm font-semibold text-accent hover:underline">All attractions <ArrowRight className="h-4 w-4" /></Link>
        </div>
      ...
      <section className="mx-auto mt-14 max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href="/amsterdam/restaurants" className="inline-flex items-center gap-1.5 text-sm font-semibold text-accent hover:underline">All restaurants <ArrowRight className="h-4 w-4" /></Link>
        </div>
      ...
      <section className="mx-auto mt-14 max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href="/amsterdam/day-trips" className="inline-flex items-center gap-1.5 text-sm font-semibold text-accent hover:underline">All day trips <ArrowRight className="h-4 w-4" /></Link>
        </div>
      ...
      <section id="nearby" className="mx-auto mt-16 max-w-7xl scroll-mt-32 px-4 sm:px-6 lg:px-8">
        ...
        <h2 className="mt-2 font-serif text-3xl font-semibold text-foreground sm:text-4xl">Fly onward from Amsterdam</h2>
        <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {AMSTERDAM.nearby.map((n) => (
      ...
      <section className="mx-auto mt-16 max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
        <p className="text-xs font-semibold uppercase tracking-[0.25em] text-accent">Amsterdam guides</p>
        <div className="mt-6 grid grid-cols-2 gap-x-6 gap-y-3 text-sm sm:grid-cols-3 lg:grid-cols-4">
          {categories.map((c) => (
            <Link key={c.slug} href={`/amsterdam/${c.slug}`} className="text-muted-foreground transition hover:text-accent">
              Amsterdam {c.label.toLowerCase()} →
            </Link>
          ))}
          {AMSTERDAM.nearby.map((n) => (
